...
export default function StudentsListPage() {
  ...
  const handleViewHistory = async (student: any) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/transactions/student/${student.id}`, {
        headers: getAuthHeaders()
      });
  ...
  const handleDeleteHistory = async () => {
    ...
    try {
       const res = await fetch(`http://localhost:5000/api/transactions/student/${selectedStudent.id}`, {
         method: 'DELETE',
         headers: getAuthHeaders()
       });
       if (res.ok) {
